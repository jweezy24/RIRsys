...
def plot_all_test(tests,plot_ffts=True):
    ...
    for ax1,ax2,ax3,ax4,ax5 in axs:
        i = tests[count]
        if plot_ffts:
            IR_est_1 =  normalize(abs(rfft(data[f"IR_est_{i}_1"])))
            IR_est_2 =  normalize(abs(rfft(data[f"IR_est_{i}_2"])))
            IR_est_3 =  normalize(abs(rfft(data[f"IR_est_{i}_3"])))
            orig =  normalize(abs(rfft(data[f"orig"])))
